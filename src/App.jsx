...
export default function App() {
  ...
  */
  ...
  const refetchAll = async () => {
    try {
      const [sts, pys, cfg] = await Promise.all([api.listStudents(), api.listPayments(), api.getSettings()]);
      setStudents(sts);
      setPayments(pys);
  ...
  useEffect(() => {
    ...
    const payCh = supabase.channel('rt-payments').on('postgres_changes', { event: '*', schema: 'public', table: 'payments' }, async () => {
      try { const pys = await api.listPayments(); setPayments(pys); } catch {}
    }).subscribe();
  ...
  const promote = async (ids, targetClass, sectionAssignment, renumber) => {
    try {
      const targetSection = SECTION_CLASSES.includes(targetClass) ? sectionAssignment : null;
      // Update each promoted student
      await Promise.all(ids.map(id => api.updateStudentFields(id, { studentClass: targetClass, section: targetSection })));

      // Optimistic update
      let next = students.map(s => ids.includes(s.id) ? { ...s, studentClass: targetClass, section: targetSection } : s);
      ...
      if (renumber) {
        ...
        // Apply roll renumber to DB for affected students
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        {page === 'export' && <Export stats={stats} settings={settings} payments={payments} byClass={byClass} onCWStudents={expCWStudents} onAllStudents={expAllStudents} onCWPayments={expCWPayments} onAllPayments={expAllPayments} onFeeRegister={expFeeRegister} onFeeRegisterClass={expFeeRegisterClass} />}
...
function Export({ stats, settings, payments, byClass, onCWStudents, onAllStudents, onCWPayments, onAllPayments, onFeeRegister, onFeeRegisterClass }) {
  const years = useMemo(() => {
...
function PromoteModal({ byClass, onClose, onPromote }) {
  ...
  const can = src && tgt && src !== tgt && selIds.length > 0 && (!needsSection || section);
  const submit = async () => {
    if (!can || busy) return;
    setBusy(true);
    await onPromote(selIds, tgt, needsSection ? section : null, renumber);
    setBusy(false);
  };
